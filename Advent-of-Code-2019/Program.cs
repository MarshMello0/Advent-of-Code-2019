...
namespace Advent_of_Code_2019
{
    class Program
    {
        ...
        #region Day One
        //private static int[] DayOneInput { set; get; } = new int[] { 14,1969,100756 };
        ...
        private static void Day1()
        {
            ...
            int lastMass,lastAddonMass;
            int result = 0;
            int resultTwo = 0;
            for (int i = 0; i < DayOneInput.Length; i++)
            {
                lastMass = (int)(MathF.Floor(DayOneInput[i] / 3f) - 2f);
                result += lastMass;
                resultTwo += lastMass;
                lastAddonMass = lastMass;
                while (lastAddonMass > 0)
                {
                    lastAddonMass = (int)(MathF.Floor(lastAddonMass / 3f) - 2f);
                    if (lastAddonMass > 0)
                    {
                        resultTwo += lastAddonMass;
                    }
                }
            }
